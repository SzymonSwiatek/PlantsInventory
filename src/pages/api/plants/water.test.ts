...
function fakeContext(body?: unknown, user?: User | null): APIContext {
  return {
    ...
    params: {},
    cookies: {},
    request: new Request("http://test.local/api/plants/water", {
      method: "POST",
...
describe("/api/plants/water — POST validation", () => {
  beforeEach(() => {
...
describe("/api/plants/water — POST happy path", () => {
  it("inserts a care_event and updates the plant, returns watered ids", async () => {
    const client = mockClient();
    const res = await POST(fakeContext({ plantIds: [PLANT_A] }));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ watered: [PLANT_A] });

    const insertArg = client._insertFn.mock.calls[0][0] as Record<string, unknown>[];
    expect(insertArg).toHaveLength(1);
    expect(insertArg[0]).toMatchObject({ plant_id: PLANT_A, kind: "water" });
    expect(typeof insertArg[0].done_at).toBe("string");

    const updateArg = client._updateFn.mock.calls[0][0] as Record<string, unknown>;
    expect(updateArg).toMatchObject({ water_snooze_until: null });
    expect(typeof updateArg.last_watered_at).toBe("string");
  ...
  it("marks all ids in a bulk request", async () => {
    ...
    expect(res.status).toBe(200);
    const body = (await res.json()) as { watered: string[] };
    expect(body.watered).toEqual([PLANT_A, PLANT_B]);
