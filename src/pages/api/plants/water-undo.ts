...
export const POST: APIRoute = async (context) => {
  ...
  // Fetch water care_events for these plants ordered newest-first.
  ...
  if (fetchError) {
    ...
    console.error("[water-undo] fetch care_events failed:", fetchError);
  ...
  if (toDeleteIds.length > 0) {
    ...
    if (deleteError) {
      ...
      // eslint-disable-next-line no-console
      console.error("[water-undo] delete care_events failed:", deleteError);
      return json({ error: "delete_failed" }, 500);
    }
  }

  // Update each plant's last_watered_at to the prior event's done_at (or null).
  // Each plant may have a different prior value, so we iterate individually.
  const reverted: string[] = [];
  for (const plantId of plantIds) {
    const entry = byPlant.get(plantId);
    const lastWateredAt = entry?.priorDoneAt ?? null;
    ...
    if (updateError) {
      ...
      // eslint-disable-next-line no-console
      console.error("[water-undo] update plant failed:", updateError);
