...
function fakeContext(body?: unknown, user?: User | null): APIContext {
  return {
    ...
    params: {},
    cookies: {},
    request: new Request("http://test.local/api/plants/water-undo", {
      method: "POST",
...
describe("/api/plants/water-undo — POST validation", () => {
  beforeEach(() => {
...
describe("/api/plants/water-undo — POST undo logic", () => {
  it("restores last_watered_at to the prior event's done_at", async () => {
    ...
    expect(updateArg).toEqual({ last_watered_at: PRIOR_DONE_AT });
    expect(client._updateEq).toHaveBeenCalledWith("id", PLANT_A);
  });

  it("sets last_watered_at to null when there is no prior event", async () => {
    ...
    const updateArg = client._updateFn.mock.calls[0][0] as Record<string, unknown>;
    expect(updateArg).toEqual({ last_watered_at: null });
  });

  it("is a no-op (last_watered_at → null) when no water events exist for the plant", async () => {
  ...
  it("handles bulk undo: each plant gets its own prior done_at", async () => {
    ...
    expect(updateCalls[0][0]).toEqual({ last_watered_at: PRIOR_DONE_AT });
    expect(updateCalls[1][0]).toEqual({ last_watered_at: PRIOR_B });
  });
});
