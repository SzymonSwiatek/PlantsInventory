...
export const POST: APIRoute = async (context) => {
  ...
  const { error: insertError } = await supabase
    .from("care_events")
    .insert(plantIds.map((plantId) => ({ plant_id: plantId, kind: "winterize" as const, done_at: now })));
  if (insertError) {
    if (insertError.code && CLIENT_ERROR_CODES.has(insertError.code)) {
      return json({ error: "invalid_request" }, 400);
    }
    // eslint-disable-next-line no-console
    console.error("[winterize] insert care_events failed:", insertError);
    return json({ error: "insert_failed" }, 500);
  }

  const { error: updateError } = await supabase.from("plants").update({ winterized_at: now }).in("id", plantIds);
  if (updateError) {
    if (updateError.code && CLIENT_ERROR_CODES.has(updateError.code)) {
      return json({ error: "invalid_request" }, 400);
    }
    // eslint-disable-next-line no-console
    console.error("[winterize] update plants failed:", updateError);
    return json({ error: "update_failed" }, 500);
  }

  return json({ winterized: plantIds });
};
